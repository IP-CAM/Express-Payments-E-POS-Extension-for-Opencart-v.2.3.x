...
class ControllerExtensionPaymentEposExpressPay extends Controller
{
	public function index()
	{
		...
		$data['redirect'] = $this->url->link('extension/payment/epos_expresspay/send');
	...
	public function send()
	{
		...
		$secret_word = $this->config->get('epos_expresspay_secret_key');
		$is_use_signature = ($this->config->get('epos_expresspay_sign_invoices') == 'on') ? true : false;

		$url = ($this->config->get('epos_expresspay_test_mode') != 'on') ? $this->config->get('epos_expresspay_url_api') : $this->config->get('epos_expresspay_url_sandbox_api');
		$url .= "/v1/invoices?token=" . $this->config->get('epos_expresspay_token');
		...
		$request_params = array(
			...
			"IsNameEditable" => (($this->config->get('epos_expresspay_name_editable') == 'on') ? 1 : 0),
			"IsAddressEditable" => (($this->config->get('epos_expresspay_address_editable') == 'on') ? 1 : 0),
			"IsAmountEditable" => (($this->config->get('epos_expresspay_amount_editable') == 'on') ? 1 : 0)
		);

		if ($is_use_signature)
			$url .= "&signature=" . $this->computeSignature($request_params, $secret_word);
		...
		try {
			...
			$this->redirect($this->url->link('extension/payment/epos_expresspay/fail'));
		}
		...
		if (isset($response->InvoiceNo)) {
			...
			$data['test_mode'] = ($this->config->get('epos_expresspay_test_mode') == 'on') ? true : false;
			$data['message_success'] = nl2br($this->config->get('epos_expresspay_message_success'), true);
			...
			if ($data['is_use_signature']) {
				$secret_word = $this->config->get('epos_expresspay_secret_key_notify');
			...
			$request_params = array(
				"InvoiceId" =>  $response->InvoiceNo,
				'ViewType' => 'base64'
			);
			$secret_word = $this->config->get('epos_expresspay_secret_key');
			$request_params["Signature"] = $this->computeSignature($request_params, $secret_word, "get-qr-code");

			$token= $this->config->get('epos_expresspay_token'); 

			$request_params = http_build_query($request_params);

			$url = "https://api.express-pay.by/v1/qrcode/getqrcode/?token=$token&" . $request_params;

			$this->log_info('success',$url);
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			...
			if (isset($response->QrCodeBody)) {
				...
				$data['qr_code'] = "";
				$data['qr_description'] = 'Ошибка получения Qr-кода';
			}

			$this->load->model('checkout/order');
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('epos_expresspay_pending_status_id'));
			...
			$this->response->setOutput($this->load->view('extension/payment/epos_expresspay_success.tpl', $data));
		} else
			$this->response->redirect($this->url->link('extension/payment/epos_expresspay/fail'));
	}
	...
	public function fail()
	{
		...
		$this->load->model('checkout/order');
		$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('epos_expresspay_cancel_status_id'));
		...
		$this->response->setOutput($this->load->view('extension/payment/epos_expresspay_failure.tpl', $data));
	}
	...
	public function notify()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$secret_word = $this->config->get('epos_expresspay_secret_key_notify');
			$is_use_signature = ($this->config->get('epos_expresspay_sign_notify') == 'on') ? true : false;
	...
	private function notify_success($dataJSON)
	{
		...
		if (isset($data->CmdType)) {
			switch ($data->CmdType) {
				case '1':
					$this->model_checkout_order->addOrderHistory($data->AccountNo, $this->config->get('epos_expresspay_processing_status_id'));
					$this->log_info('notify_success', 'Initialization to update status. STATUS ID - ' . $this->config->get('epos_processing_status_id') . "; RESPONSE - " . $dataJSON);

					break;
				case '2':
					$this->model_checkout_order->addOrderHistory($data->AccountNo, $this->config->get('epos_expresspay_cancel_status_id'));
					$this->log_info('notify_success', 'Initialization to update status. STATUS ID - ' . $this->config->get('epos_cancel_status_id') . "; RESPONSE - " . $dataJSON);
	...
	function computeSignature($requestParams, $secretWord, $method = "add-invoice")
	{
		...
		$result = $this->config->get('epos_expresspay_token');
		foreach ($apiMethod as $item) {
			$result .= $normalizedParams[$item];
		}
		$this->log_info('success', $result);
		$this->log_info('success', $secretWord);
